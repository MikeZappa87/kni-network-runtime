...
func (syncer *cniNetConfSyncer) syncLoop() error {
	for {
		...
		case event, ok := <-syncer.watcher.Events:
			if !ok {
				log.Debugf("cni watcher channel is closed")
				return nil
			}
			...
			if event.Has(fsnotify.Chmod) || event.Has(fsnotify.Create) {
				log.Debugf("ignore event from cni conf dir: %s", event)
				continue
			}
			log.Debugf("receiving change event from cni conf dir: %s", event)
		...
		case err := <-syncer.watcher.Errors:
			if err != nil {
				log.WithError(err).Error("failed to continue sync cni conf change")
				return err
			}
		}
	}
